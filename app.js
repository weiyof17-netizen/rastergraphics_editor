...
function safeSetPointerCapture(pointerId) {
  try {
    ...
    // Pointer capture can fail when the browser cancels the pointer stream.
  }
}

function safeReleasePointerCapture(pointerId) {
  try {
    if (Number.isInteger(pointerId) && els.canvas.hasPointerCapture(pointerId)) {
      els.canvas.releasePointerCapture(pointerId);
    }
  } catch {
    // A stale pointer id should not block the editor from continuing to work.
  }
}

function isActivePointer(event) {
  return !state.drag || state.drag.pointerId === event.pointerId;
}

function cancelCanvasAction(message = "Действие прервано") {
  if (!state.drag) return;
  const pointerId = state.drag.pointerId;
  state.drag = null;
  safeReleasePointerCapture(pointerId);
  render();
  setStatus(message);
}
...
function startCanvasAction(event) {
  ...
  cancelCanvasAction("Предыдущее действие завершено");
...
function finishCanvasAction(event) {
  if (!state.drag) return;
  if (event.pointerId !== undefined && !isActivePointer(event)) return;
  event.preventDefault();
...
window.addEventListener("pointerup", finishCanvasAction);
window.addEventListener("pointercancel", () => cancelCanvasAction());
window.addEventListener("blur", () => cancelCanvasAction("Действие остановлено"));
